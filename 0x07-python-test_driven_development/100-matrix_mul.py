...
def matrix_mul(m_a, m_b):
    """
    matrix_mul function
    prints a result of matrix multiplaction of two matricies
...
def check_list_of_lists(matrix):
    """checks if a list is a list of lists that is matrix
    """
    for row in matrix:
...
def check_ele_types(matrix):
    """checking if a matrix contains non ints or floats
    """
    for row in matrix:
        for ele in row:
...
def check_empty_list(m):
    """checking if the matrix is empty of sub matrix is empty
    """
...
def check_rectangle(matrix):
    """checking if the matrix is a rectangle
    """
    prevRowSize = -1
    for row in matrix:
